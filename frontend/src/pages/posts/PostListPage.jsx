    ...
    <div className="community-page"><CommunityHeader />
      <div className="community-shell">
        ...
        <section className="feed-column" aria-live="polite">
          ...
          <div className="feed-heading"><div><h2>{selectedCategory ? `Bài đăng trong ${selectedCategory.name}` : feed === 'home' ? 'Dành cho bạn' : feed === 'popular' ? 'Phổ biến trên VRUM' : 'Tất cả bài đăng'}</h2><p>{totalPosts} bài đăng</p></div><CustomSelect className="feed-sort-menu" compact value={sort} onChange={(value) => setSearchParams((current) => { const next = new URLSearchParams(current); next.set('sort', value); return next; })} ariaLabel="Sắp xếp bài viết" options={[{ value: 'hot', label: 'Nổi bật' }, { value: 'new', label: 'Mới nhất' }, { value: 'top', label: 'Điểm cao' }]} /></div>

...
export default function PostListPage() {
  ...
  const authorName = searchParams.get('authorName') || 'thành viên này';
  ...
  const posts = usePosts({ limit: 10, categoryId: categoryId || undefined, authorId: authorId || undefined, feed: categoryId ? undefined : feed, sort });
  ...
  const feedScope = `${feed}:${categoryId}:${authorId}`;
  ...
  useEffect(() => {
    if (authorId) window.scrollTo({ top: 0, behavior: 'instant' });
  }, [authorId, categoryId]);
    ...
    <div className="community-page"><CommunityHeader />
      <div className="community-shell">
        ...
        <section className="feed-column" aria-live="polite">
          ...
          <div className="feed-heading"><div><h2>{authorId ? `Bài đăng của ${loadedPosts[0]?.author?.fullName || loadedPosts[0]?.author?.username || authorName}` : selectedCategory ? `Bài đăng trong ${selectedCategory.name}` : feed === 'home' ? 'Dành cho bạn' : feed === 'popular' ? 'Phổ biến trên VRUM' : 'Tất cả bài đăng'}</h2><p>{totalPosts} bài đăng{authorId && selectedCategory ? ` trong ${selectedCategory.name}` : ''}</p>{authorId && <Link to={`/posts?${new URLSearchParams(categoryId ? { categoryId } : { feed: 'all' })}`}>Xem tất cả bài đăng trong cộng đồng</Link>}</div><CustomSelect className="feed-sort-menu" compact value={sort} onChange={(value) => setSearchParams((current) => { const next = new URLSearchParams(current); next.set('sort', value); return next; })} ariaLabel="Sắp xếp bài viết" options={[{ value: 'hot', label: 'Nổi bật' }, { value: 'new', label: 'Mới nhất' }, { value: 'top', label: 'Điểm cao' }]} /></div>
          ...
          {!posts.isLoading && !posts.isError && loadedPosts.length === 0 && <div className="feed-state empty-state"><Sparkles size={28} /><h3>Chưa có bài đăng</h3><p>{authorId ? 'Người này chưa có bài đăng hiển thị trong cộng đồng này.' : selectedCategory ? 'Hãy là người đầu tiên chia sẻ trong chủ đề này.' : 'Bài đăng mới sẽ xuất hiện tại đây.'}</p>{!authorId && createPostUrl && <Link to={user ? createPostUrl : '/login'} state={user ? undefined : { from: createPostUrl }}>Tạo bài đăng đầu tiên</Link>}</div>}

...
import { ArrowLeft, ArrowRight, BellOff, BellRing, Check, PenLine, Plus, Sparkles, Star, Trash2 } from 'lucide-react';
...
export default function PostListPage() {
  ...
  const posts = usePosts({ page, limit: 8, categoryId: categoryId || undefined, feed: categoryId ? undefined : feed, sort });
    ...
    <div className="community-page"><CommunityHeader />
      <div className="community-shell">
        ...
        <section className="feed-column" aria-live="polite">
          ...
          <div className="feed-heading"><div><h2>{selectedCategory ? `Bài đăng trong ${selectedCategory.name}` : feed === 'home' ? 'Dành cho bạn' : feed === 'popular' ? 'Phổ biến trên VRUM' : 'Tất cả bài đăng'}</h2><p>{posts.data?.meta.total || 0} bài đăng</p></div><select className="feed-sort" value={sort} onChange={(event) => setSearchParams((current) => { const next = new URLSearchParams(current); next.set('sort', event.target.value); return next; })} aria-label="Sắp xếp bài viết"><option value="hot">Nổi bật</option><option value="new">Mới nhất</option><option value="top">Điểm cao</option></select></div>
          ...
          {posts.data?.data.filter((post) => !dismissedPosts.some((item) => item.id === post.id && item.scope === feedScope)).map((post) => <PostCard key={post.id} post={post} onHidden={setLastHiddenPost} onNotInterested={(id) => setDismissedPosts((current) => [...current, { id, scope: feedScope }])} />)}
          {!posts.isLoading && posts.data?.data.length === 0 && <div className="feed-state empty-state"><Sparkles size={28} /><h3>Chưa có bài đăng</h3><p>{selectedCategory ? 'Hãy là người đầu tiên chia sẻ trong chủ đề này.' : 'Bài đăng mới sẽ xuất hiện tại đây.'}</p>{createPostUrl && <Link to={user ? createPostUrl : '/login'} state={user ? undefined : { from: createPostUrl }}>Tạo bài đăng đầu tiên</Link>}</div>}

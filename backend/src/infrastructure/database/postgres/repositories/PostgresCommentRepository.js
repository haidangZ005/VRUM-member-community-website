...
const selectComment = `
  SELECT cm.*, p.title AS post_title, u.username AS author_username, u.full_name AS author_full_name, u.avatar_url AS author_avatar_url,
...
class PostgresCommentRepository {
  async create(comment, database = pool) {
    ...
    const result = await database.query(`${selectComment} WHERE cm.id = $2`, [comment.authorId, rows[0].id]);
    return mapComment(result.rows[0]);
  }

  async listByPost(postId, viewerId = null) {
    const { rows } = await pool.query(
      `${selectComment} WHERE cm.post_id = $2 AND cm.status = 'visible' ORDER BY cm.created_at ASC`,
      [viewerId, postId],
    );
  ...
  async listAll({ page, limit, search, status }) {
    ...
    const [itemsResult, countResult] = await Promise.all([
      pool.query(
        `${selectComment} WHERE ($2 = '' OR cm.content ILIKE $3 OR u.username ILIKE $3 OR p.title ILIKE $3)
